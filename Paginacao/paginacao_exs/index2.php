...
<?php
	...
	try {
     $dbh = new PDO("mysql:host=$hostname;dbname=auto_crud", $username, $password);
    }
    ...
    $STM = $dbh->prepare("SELECT id FROM clientes WHERE nome = :nome");
	...
	$STM2 = $dbh->prepare("SELECT * FROM clientes WHERE nome = :nome ORDER BY id LIMIT $start, $limit");
