...
//combo($tableFk, $idFk, $displayFk, $selected=null)
    public function formFieldsInsert($tableFk, $idFk, $displayFk, $selected){
	    $fields = '';
        ...
        for($x=1;$x < $this->numFields();$x++){
		    ...
		    if($x < $this->numFields()){
                if($field == $selected) {
                    $fields .= $this->combo($tableFk, $idFk, $displayFk, $selected)."\n";
                }else{
    ...
    public function formFieldsUpdate($tableFk, $idFk, $displayFk, $selected, $tablePk, $id){
	    $fields = '';
        $sth = $this->pdo->prepare("SELECT * from $tablePk where id = :id");
        $sth->bindValue(':id', $id, PDO::PARAM_INT); // No select e no delete basta um bindValue, mas isso quando é exigido por :id
        $sth->execute();
        ...
        for($x=1;$x < $this->numFields();$x++){
		    ...
		    if($x < $this->numFields()){
                if($field == $selected) {
                    $fields .= $this->combo($tableFk, $idFk, $displayFk, $selected, $tablePk,$id)."\n";
                }else{
    ...
    public function combo($tableFk, $idFk, $displayFk, $selected, $tablePk=null,$id=null){ // $sql='select id,descricao from produtos'
