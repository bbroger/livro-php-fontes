...
class crud extends Connection
{
    ...
    public function formFieldsInsert($tableFk, $tablePk, $idFk, $display, $selected){
	    $fields = '';
        ...
        for($x=1;$x < $this->numFields();$x++){
		    ...
		    if($x < $this->numFields()){
                if($field == $fk) {
                    $fields .= $this->combo($tableFk, $tablePk, $idFk, $display, $selected)."\n";
                }else{
    ...
    public function formFieldsUpdate($tableFk, $tablePk, $idFk, $fk, $display){
	    $fields = '';
        $sth = $this->pdo->prepare("SELECT * from $tablePk");
        $sth->execute();
        ...
        for($x=1;$x < $this->numFields();$x++){
		    ...
		    if($x < $this->numFields()){
                if($field == $fk) {
                    $fields .= $this->combo($tableFk, $tablePk, $idFk, $fk, $display)."\n";
                }else{
    ...
    // Preenche uma combo com dados de uma tabela estrangeira, para ser usado em vez de um campo text com chave estrangeira. Ex: category_id    
    public function combo($tableFk, $tablePk, $idFk, $display, $selected){

        $sql = 'select '.$idFk.','.$display.' from '.$tableFk;
        $smt = $this->pdo->prepare($sql);
        $smt->execute();
        $data = $smt->fetchAll();

        $sql2 = 'select '.$selected.' from '.$tablePk;
        $smt2 = $this->pdo->prepare($sql2);
        $smt2->execute();
        $data2 = $smt2->fetchAll();

require 'uteis.php';
//d($data2[0]['category_id']);

        foreach($data2 as $cat){
            $cat = end($cat);
        }

            $combo = "<tr><td><b>".ucfirst($fk)."</b></td><td><select name=".$fk." class=\"form-control\">\n";
            foreach ($data as $row){
//print $row[$idFk] .'=='. $cat.'-';
                if($row[$idFk] == $selected){
                    $combo .= "<option value=\"".$cat."\" selected>".$row["$display"]."</option>\n";
                }else{
//                    print '<option>Selecione</option>';
                    $combo .= "<option value=\"".$row[$idFk]."\">".$row["$display"]."</option>\n";
                }
            }
        $combo .= '</select></td></tr>';
//exit;
